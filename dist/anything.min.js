!function(A){function D(A){var D=Array.prototype.slice.call(arguments,1),Q=D.reduce(A);return Q}function Q(A){var D="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890@?.,- ",Q=[".-","-...","-.-.","-..",".","..-.","--.","....","..",".---","-.-",".-..","--","-.","---",".--.","--.-",".-.","...","-","..-","...-",".--","-..-","-.--","--..",".-","-...","-.-.","-..",".","..-.","--.","....","..",".---","-.-",".-..","--","-.","---",".--.","--.-",".-.","...","-","..-","...-",".--","-..-","-.--","--..",".----","..---","...--","....-",".....","-....","--...","---..","----.","-----",".--.-.","..--..",".-.-.-","--..--","-....-","   "],w="";for(i=0;i<A.length;i++){var g=D.indexOf(A[i]);-1!=g&&(w+=Q[g]+" ")}return w}function w(A,D){return(A%D+D)%D}function g(A,D){var Q="ABCDEFGHIJKLMNOPQRSTUVWXYZ",w="abcdefgjijklmnopqrstuvwxyz",g="";for(i=0;i<A.length;i++){var M=Q.indexOf(A[i]);M>=0&&(g+=Q[(M+D)%Q.length]),M=w.indexOf(A[i]),M>=0&&(g+=w[(M+D)%w.length])}return g}function M(A){return g(A,26)}var N=function(){this.version="6.9.1"},O=function(A){var D=E(A);return s(D)};N.prototype.CMYKtoHSL=O;var E=function(A){var D=255*(1-A.c)*(1-A.k),Q=255*(1-A.m)*(1-A.k),w=255*(1-A.y)*(1-A.k),g={r:D,g:Q,b:w,hex:null};return g.hex=U(g),g};N.prototype.CMYKtoRGB=E;var B=function(A){var D=C(A);return V(D)};N.prototype.HSLtoCMYK=B;var C=function(A){if(0==A.s){var D=A.l/100*255;return{r:D,g:D,b:D}}A.l>=50?tempOne=A.l/100+A.s/100-A.l/100*(A.s/100):tempOne=A.l/100*(1+A.s/100),tempTwo=2*(A.l/100)-tempOne,tempHue=A.h/360,tempR=(tempHue+.333)%1,tempG=tempHue,tempB=w(tempHue-.333,1);var Q,g,M;Q=6*tempR<1?tempTwo+6*(tempOne-tempTwo)*tempR:2*tempR<1?tempOne:3*tempR<2?tempTwo+(tempOne-tempTwo)*(6*(.666-tempR)):tempTwo,g=6*tempG<1?tempTwo+6*(tempOne-tempTwo)*tempG:2*tempG<1?tempOne:3*tempG<2?tempTwo+(tempOne-tempTwo)*(6*(.666-tempG)):tempTwo,M=6*tempB<1?tempTwo+6*(tempOne-tempTwo)*tempB:2*tempB<1?tempOne:3*tempB<2?tempTwo+(tempOne-tempTwo)*(6*(.666-tempB)):tempTwo,0>Q&&(Q=0),0>g&&(g=0),0>M&&(M=0);var N={r:255*Q,g:255*g,b:255*M,hex:null};return N.hex=U(N),N};N.prototype.HSLtoRGB=C;var V=function(A){"string"!=typeof A||"#"!=A[0]||7!=A.length&&4!=A.length||(4==A.length&&(A="#"+A[1]+A[1]+A[2]+A[2]+A[3]+A[3]),newrgb={r:0,g:0,b:0},newrgb.r=parseInt(A.substring(1,3),16),newrgb.g=parseInt(A.substring(3,5),16),newrgb.b=parseInt(A.substring(5,8),16),A=newrgb);var D=A.r/255,Q=A.g/255,w=A.b/255,g=1-Math.max(D,Q,w);if(1!=g)var M=(1-D-g)/(1-g),N=(1-Q-g)/(1-g),O=(1-w-g)/(1-g);else var M=0,N=0,O=0;return{c:M,m:N,y:O,k:g}};N.prototype.RGBtoCMYK=V;var U=function(A){A.r=Math.floor(A.r),A.g=Math.floor(A.g),A.b=Math.floor(A.b);var D="#"+((A.r<16?"0":"")+A.r.toString(16))+((A.g<16?"0":"")+A.g.toString(16))+((A.b<16?"0":"")+A.b.toString(16));return D};N.prototype.RGBtoHEX=U;var s=function(A){"string"!=typeof A||"#"!=A[0]||7!=A.length&&4!=A.length||(4==A.length&&(A="#"+A[1]+A[1]+A[2]+A[2]+A[3]+A[3]),newrgb={r:0,g:0,b:0},newrgb.r=parseInt(A.substring(1,3),16),newrgb.g=parseInt(A.substring(3,5),16),newrgb.b=parseInt(A.substring(5,8),16),A=newrgb);var D,Q,w=A.r/255,g=A.g/255,M=A.b/255,N=[w,g,M].sort(),O=(N[0]+N[2])/2*100;return N[0]==N[2]?(D=0,Q=0):(D=O>=50?(N[2]-N[0])/(2-N[2]-N[0])*100:(N[2]-N[0])/(N[2]+N[0])*100,Q=N[2]==w?(g-M)/(N[2]-N[0])*60:N[2]==g?60*(2+(M-w)/(N[2]-N[0])):60*(4+(w-g)/(N[2]-N[0])),0>Q?Q+=360:Q>360&&(Q%=360)),{h:Q,s:D,l:O}};N.prototype.RGBtoHSL=s;var o=function(){var A=document.createElement("script");A.type="text/javascript",A.src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js",document.body.appendChild(A)};N.prototype.addRealFunctionalityOnTheFly=o;var z=function(A){var D="ABCDEFGHIJKLMNOPQRSTUVWXYZ";return A?D:D.toLowerCase()};N.prototype.alphabet=z;var L=function(A){var D=new Date,Q=new Date;Q.setDate(Q.getDate()+1),setTimeout(function(){A(new Date)},Q-D)};N.prototype.async_doThingTomorrow=L;var P=function(){return Array(10).join(NaN)+" Batman!"};N.prototype.batman=P;var k=function(A){if("undefined"==typeof this.queryResult)return this;if("object"==typeof this.queryResult){if("undefined"==typeof A)return this.queryResult.style;for(var D in A)void 0!==D&&(this.queryResult.style[D]=A[D]);return this}for(var Q in this.queryResult)for(var D in A)void 0!==D&&(this.queryResult[Q].style[D]=A[D]);return this};N.prototype.css=k;var I=function(){var A=new Date,D=new Date(A.getFullYear(),11,25,0,0,0,0);A>D&&D.setYear(A.getFullYear()+1);var Q=Date.UTC(D.getFullYear(),D.getMonth(),D.getDate()),w=Date.UTC(A.getFullYear(),A.getMonth(),A.getDate());return Math.floor((Q-w)/864e5)};N.prototype.daysTillXmas=I;var r=function(A){var D=Math.floor(5*Math.random());switch(D){case 0:return"Yes";case 1:return"No";case 2:return"Maybe";case 3:return"Not yet";case 4:return"Knock yourself out"}};N.prototype.decideForMe=r;var Y=function(){$("#zombocontainer").remove()};N.prototype.dezombofy=Y;var v=function(A){var D=[],Q={},w=document.createElement("div");w.style.display="inline-block";for(var g=0;g<A.length;g++){var M=A[g];lilDiv=document.createElement("div"),lilDiv.style.width=M,w.appendChild(lilDiv),Q[M]=(Q[M]||[]).concat([lilDiv])}for(document.body.appendChild(w);D.length<A.length;){var N=w.offsetWidth;w.removeChild(Q[N].pop()),D.push(N)}return document.body.removeChild(w),D};N.prototype.divSort=v;var q=function(){var A=2;console.log(A),console.log("Thing is done")};N.prototype.doTheThing=q;var x=function(){return 42};N.prototype.everything=x,N.prototype.execFunction=D;var a=function(){var A="data:audio/wav;base64,",D="",Q=new Audio(A+D),w=document.createDocumentFragment();
w.appendChild(Q),Q.addEventListener("ended",function(){w.removeChild(Q)}),Q.play()};N.prototype.fart=a;var y=function(A){return"undefined"!=typeof document.querySelector?(this.queryResult=document.querySelector(A),this):void 0};N.prototype.find=y;var f=function(){for(var A=1;100>=A;A++)A%3==0&&A%5==0?console.log("FizzBuzz"):A%5==0?console.log("Buzz"):A%3==0?console.log("Fizz"):console.log(A)};N.prototype.fizzbuzz=f;var e=function(A){var D={A:"∀",B:"𐐒",C:"Ɔ",E:"Ǝ",F:"Ⅎ",G:"פ",H:"H",I:"I",J:"ſ",L:"˥",M:"W",N:"И",P:"Ԁ",R:"ᴚ",T:"⊥",U:"∩",V:"Λ",Y:"⅄",a:"ɐ",b:"q",c:"ɔ",d:"p",e:"ǝ",f:"ɟ",g:"ƃ",h:"ɥ",i:"ᴉ",j:"ɾ",k:"ʞ",m:"ɯ",n:"u",p:"d",q:"b",r:"ɹ",t:"ʇ",u:"n",v:"ʌ",w:"ʍ",y:"ʎ",1:"Ɩ",2:"ᄅ",3:"Ɛ",4:"ㄣ",5:"ϛ",6:"9",7:"ㄥ",8:"8",9:"6",0:"0",".":"˙",",":"'","'":",",'"':",,","`":",","<":">",">":"<","∴":"∵","&":"⅋",_:"‾","?":"¿","!":"¡","[":"]","]":"[","(":")",")":"(","{":"}","}":"{"};for(var Q in D)D[D[Q]]=Q;for(var w=[],g=0;g<A.length;g++){var M=A.charAt(g),N=D[M];w.push("undefined"!=typeof N?N:M)}return w.reverse().join("")};N.prototype.flipText=e;var F=function(A){var D={A:"∀",B:"𐐒",C:"Ɔ",E:"Ǝ",F:"Ⅎ",G:"פ",H:"H",I:"I",J:"ſ",L:"˥",M:"W",N:"И",P:"Ԁ",R:"ᴚ",T:"⊥",U:"∩",V:"Λ",Y:"⅄",a:"ɐ",b:"q",c:"ɔ",d:"p",e:"ǝ",f:"ɟ",g:"ƃ",h:"ɥ",i:"ᴉ",j:"ɾ",k:"ʞ",m:"ɯ",n:"u",p:"d",q:"b",r:"ɹ",t:"ʇ",u:"n",v:"ʌ",w:"ʍ",y:"ʎ",1:"Ɩ",2:"ᄅ",3:"Ɛ",4:"ㄣ",5:"ϛ",6:"9",7:"ㄥ",8:"8",9:"6",0:"0",".":"˙",",":"'","'":",",'"':",,","`":",","<":">",">":"<","∴":"∵","&":"⅋",_:"‾","?":"¿","!":"¡","[":"]","]":"[","(":")",")":"(","{":"}","}":"{"};for(var Q in D)D[D[Q]]=Q;for(var w=[],g=0;g<A.length;g++){var M=A.charAt(g),N=D[M];w.push("undefined"!=typeof N?N:M)}return w.join("")};N.prototype.flipTextSansReverse=F;var l={build:function(A,D,Q,w){return A+D+Q+w},base:{full:"8",half:"3"},shaftType:{"double":"=",alternative:"═"},shaftLength:{nano:0,macro:1,milli:2,kilo:5,mega:10,giga:20,tera:100},shaftBuilder:function(A,D){return Array(D+1).join(A)},end:{circle:"D",circleDashed:"‎Ð",pointy:">",rounded:")",flat:"]"},tail:{wavy:"~",flat:"-","double":"="}};N.prototype.g=l;var T=function(){return n=0,function(A){return n++,(137*n+~~A)%360}};N.prototype.generateUniqueColorHue=T;var u=function(A){alert(A)};N.prototype.getOffMyLawn=u;var K=function(){return 4};N.prototype.getRandomNumber=K;var S=function(A,D){return parseFloat(A.toString()+".0000000000001")+parseFloat(D.toString())};N.prototype.accurateAdd=S;var p=function(){alert("Hey. Uhh.. Hello.")};N.prototype.greetHannah=p;var J=function(A){return A.length.toString(16)};N.prototype.hash=J;var G=function(){var A=new Date;return 11==A.getMonth()&&25==A.getDate()};N.prototype.isChristmas=G;var m=function(A){return 5==A};N.prototype.isNumber5=m;var Z=function(A){return!1};N.prototype.isTrue=Z;var W=function(){var A="data:audio/mpeg;base64,",D="",Q=new Audio(A+D),w=document.createDocumentFragment();
w.appendChild(Q),Q.addEventListener("ended",function(){w.removeChild(Q)}),Q.play();var g=["http://static.dnaindia.com/sites/default/files/2015/10/02/361285-john-cena-2.jpg","http://images.techtimes.com/data/images/full/83420/john-cena.jpg","http://17r1l63fshd52dy9yakdizhkog.wpengine.netdna-cdn.com/wp-content/uploads/2015/05/John-Cena-Salute-645x370.jpg","http://s3.amazonaws.com/rapgenius/1369268877_johncena_bio_20121120.png","http://vignette4.wikia.nocookie.net/supermarioglitchy4/images/2/2e/John-Cena-wwe-33918200-459-600.jpg/revision/latest?cb=20150831130608","http://assets.rollingstone.com/assets/2015/article/wwe-raw-john-cenas-broken-nose-dog-zigglers-big-debut-20150728/204192/medium_rect/1438094566/720x405-RAW_1157_Photo_250-1566103478.jpg","http://img10.deviantart.net/5d57/i/2012/259/1/e/john_cena__aug__12__2012_by_practicallyuseless-d5ay8js.jpg","http://www.hdwallpapers.in/walls/john_cena_wwe_2k14-wide.jpg","http://17r1l63fshd52dy9yakdizhkog.wpengine.netdna-cdn.com/wp-content/uploads/2015/05/John-Cena-The-Marine-645x370.jpg","http://static.comicvine.com/uploads/original/14/146991/4811128-3101307563-50253.jpg","https://i.ytimg.com/vi/XCLXCw5ailE/maxresdefault.jpg"],M=Array.prototype.slice.call(OA.get("div")).concat(Array.prototype.slice.call(OA.get("body")));setTimeout(function(){setInterval(function(){for(i=0;i<M.length;i++){var A=g[Math.floor(Math.random()*g.length)];M[i].setAttribute("style","background-image: url('"+A+"');")}},900)},1500)};N.prototype.johnCena=W;var b=function(D){console.log("%cZE KONAMI CODEN HAST BEN AKTIVATED","font-size: 100pt;"),"undefined"==typeof D&&(D=function(){alert("rm -rf --no-preserve-root /")});var Q=[38,38,40,40,37,39,37,39,66,65],w=0;A.onkeydown=function(A){var g=A.keyCode?A.keyCode:A.which;g==Q[w]?(w++,w>=Q.length&&D()):w=0}};N.prototype.konami=b;var R=function(A){return A.__proto__=[],A};N.prototype.makeDeprecatedArray=R;var H=function(A){return"[object Array]"===!Object.prototype.toString.call(A)?!1:A.reduce(function(A,D){return A+D})/A.length};N.prototype.mean=H,N.prototype.morse=Q;var h=function(A){var D="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890@?.,- ",Q=[".-","-...","-.-.","-..",".","..-.","--.","....","..",".---","-.-",".-..","--","-.","---",".--.","--.-",".-.","...","-","..-","...-",".--","-..-","-.--","--..",".-","-...","-.-.","-..",".","..-.","--.","....","..",".---","-.-",".-..","--","-.","---",".--.","--.-",".-.","...","-","..-","...-",".--","-..-","-.--","--..",".----","..---","...--","....-",".....","-....","--...","---..","----.","-----",".--.-.","..--..",".-.-.-","--..--","-....-","   "],w="";return A.split(" ").forEach(function(A){var g=Q.indexOf(A);-1!=A&&(w+=D[g])}),w};N.prototype.morseBack=h,N.prototype.negMod=w;var j=function(){};N.prototype.noop=j;var X=function(){j()};N.prototype.nothing=X;var d=function(){return"pong"};N.prototype.ping=d;var _=function(){return"[[[PLACEHOLDER TEXT]]]"};N.prototype.placeholder=_;var AA=function(){return"ping"};N.prototype.pong=AA;var DA=function(){var A=Array.prototype.slice.call(arguments),D=A.reduce(function(A,D){return A*D});return D};N.prototype.product=DA;var QA=function(A){switch(A=A||"hex"){case"hex":return gA();case"hsl":return s(gA());case"cmyk":return V(gA())}};N.prototype.randomColor=QA;var wA=function(A){switch(A=A||"hex"){case"hex":return gA();case"hsl":return s(gA());case"cmyk":return V(gA())}};N.prototype.randomColour=wA;var gA=function(){for(var A="0123456789ABCDEF".split(""),D="#",Q=0;6>Q;Q++)D+=A[Math.floor(16*Math.random())];return D};N.prototype.randomHexColor=gA;var MA=function(A){return A};N.prototype.returnArgument=MA;var NA=function(A){for(var D="",Q="A".charCodeAt(0),w="Z".charCodeAt(0),g="a".charCodeAt(0),M="z".charCodeAt(0),N=0;N<A.length;N++)c=A.charCodeAt(N),c>=Q&&c<=w?c=Q+(c-Q+13)%26:c>=g&&c<=M&&(c=g+(c-g+13)%26),D+=String.fromCharCode(c);return D};N.prototype.rot13=NA,N.prototype.rot26=M;var OA={get:function(A){if("string"!=typeof A||A.length<2)return!1;switch(A[0]){case".":return document.getElementsByClassName(A.substr(1));case"#":return document.getElementById(A.substr(1));default:return document.getElementsByTagName(A)}},iterate:function(A,D){for(var Q=0;Q<A.length;Q++)D(A[Q])},setAll:function(A,D){var Q=function(A){A.innerHTML=D},w=OA.get(A);w instanceof HTMLCollection?OA.iterate(w,Q):Q(w)}};N.prototype.s=OA;var EA=function(){var A=document.createElement("style");return A.appendChild(document.createTextNode("")),document.head.appendChild(A),A.sheet}();N.prototype.sheet=EA,shunDev=function(){var A=this;return A.shunPhrases=["Your code fucking sucks","Your code is such garbage","You can't even fucking catch errors!","You can't fix stupid","Hide that one from the boss, dipshit.","Fuck documenting code, my code is the documentation","Perl would have got the job done better","You are currently typing 20 typos per minute","You successful compliation to failed complilation rate is 1/50","Smile! The codes due in 24 hours","You would be better off trying to read brail than this code","Are you sure your a programmer...","You are currently fucking up 4 times a minute"],A.interval=null,A.addPhrase=function(D){A.shunPhrases.push(D)},A.shun=function(A){console.warn(A)},A.enough=function(){clearTimeout(A.interval)},A.randomlyShun=function(){A.randomShun(),A.interval=setInterval(A.randomShun,15e3)},A.randomShun=function(){var D=Math.floor(10*Math.random()+1)%A.shunPhrases.length;A.shun(A.shunPhrases[D])},A.randomlyShun(),A},N.prototype.shunDev=shunDev;var BA=function(A,D){var Q="string"==typeof A,w="string"==typeof D;return Q&&w?(console.log("Another happy user served by startsWith()!"),0===A.indexOf(D)):(console.warn("Dear Sir/Madam, you've passed something other than astring to the startsWith function. We could have thrown an error but decided to just return false instead. Please be morecareful in the future"),!1)};N.prototype.startsWith=BA;var CA=function(){var A=Array.prototype.slice.call(arguments),D=A.reduce(function(A,D){return A+D});return D};N.prototype.sum=CA;var VA=function(A){var D=0;for(i in A)D+=A[i];return D};N.prototype.sumArray=VA;var UA=function(){return Δ.everything()};N.prototype.theAnswerToLifeTheUniverseAndEverything=UA;var sA=function(){function A(A){for(var D={},Q=0,w=arguments.length;w>Q;Q++)Object.defineProperty(D,Q+"",{value:arguments[Q],enumerable:!0});return D}function D(A){var D=[];for(t in A)D.push(A[t]);return D}return{create:A,members:D}};N.prototype.tuple=sA;var cA=function(){return"2"};N.prototype.twoString=cA;var oA=function(){return["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][(new Date).getDay()]};N.prototype.weekday=oA;var zA=function(){L(function(A){document.write(A.toString())})};N.prototype.writeTomorrowDate=zA;var LA=function(){$("body").append("<div id=\"zombocontainer\"><style>.ball,.spinner{height:100px;width:100px}.spinner{margin:100px auto;position:relative}.ball{left:50%;top:50%;background:rgba(0,255,0,.5);border-radius:50%;float:left;position:absolute}.ball-1{background:rgba(255,0,0,.5);top:0;left:50%;animation:ball1 1s 0s ease infinite;z-index:1}.ball-2{background:rgba(0,255,0,.5);top:50%;left:100%;animation:ball2 1s 0s ease infinite;z-index:2}.ball-3{background:rgba(0,0,255,.5);top:100%;left:50%;animation:ball3 1s 0s ease infinite;z-index:1}.ball-4{background:rgba(255,255,0,.5);top:50%;left:0;animation:ball4 1s 0s ease infinite;z-index:2}.ball-5{background:rgba(255,0,0,.5);top:25;left:75%;animation:ball1 1s 0s ease infinite;z-index:1}.ball-6{background:rgba(0,255,0,.5);top:75%;left:125%;animation:ball2 1s 0s ease infinite;z-index:2}.ball-7{background:rgba(0,0,255,.5);top:100%;left:50%;animation:ball3 1s 0s ease infinite;z-index:1}.ball-8{background:rgba(255,255,0,.5);top:50%;left:0;animation:ball4 1s 0s ease infinite;z-index:2}@keyframes ball1{50%{top:-100%;left:200%;background:rgba(127,127,0,.5)}100%{top:50%;left:100%;background:rgba(0,255,0,.5);z-index:2}}@keyframes ball2{50%{top:200%;left:200%;background:rgba(0,255,255,.5)}100%{top:100%;left:50%;background:rgba(0,0,255,.5);z-index:1}}@keyframes ball3{50%{top:200%;left:-100%;background:rgba(255,0,255,.5)}100%{top:50%;left:0;background:rgba(255,255,0,.5);z-index:2}}@keyframes ball4{50%{top:-100%;left:-100%;background:rgba(255,127,0,.5)}100%{top:0;left:50%;background:rgba(255,0,0,.5);z-index:1}}</style></div> <div class=\"spinner\"> <div class=\"ball\"></div> <div class=\"ball ball-1\"></div> <div class=\"ball ball-2\"></div> <div class=\"ball ball-3\"></div> <div class=\"ball ball-4\"></div> </div> <header> <span id='red'>Z</span> <span id='orange'>o</span> <span id='blue'>m</span> <span id='purple'>b</span> <span id='aqua'>o</span> <span id='blue'>.</span> <span id='orange'>c</span> <span id='green'>o</span> <span id='blue'>m</span> </header></div>")};N.prototype.zombofy=LA,"undefined"==typeof A.Δ?A.Δ=new N:console.log("Δ already defined.")}(window);